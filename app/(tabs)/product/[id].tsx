import { StyleSheet, View, Text, Image, ScrollView, Pressable, Alert, Platform } from 'react-native';
...
export default function ProductDetailScreen() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      setProduct({
        ...
        category: 'Túi Xách',
        description: 'Chiếc túi xách sang trọng này được chế tạo từ chất liệu cao cấp và có ngăn chứa rộng rãi với nhiều ngăn nhỏ. Hoàn hảo cho cả dịp thường ngày và trang trọng.',
        material: 'Da Thật',
        color: 'Đen',
        size: '30 x 23 x 13 cm',
        reviews: [
          { id: '1', user: 'Nguyễn Thị A', rating: 5, comment: 'Túi rất đẹp! Chất lượng tốt.' },
          { id: '2', user: 'Trần Thị B', rating: 4, comment: 'Tôi thích túi này, nhưng mong muốn có thêm nhiều ngăn hơn.' },
          { id: '3', user: 'Lê Thị C', rating: 5, comment: 'Kích thước hoàn hảo và rất bền.' }
        ]
      });
    } finally {
      setLoading(false);
  ...
  return (
    <ScrollView style={styles.container}>
      <Image source={{ uri: getProductImage() }} style={styles.image} resizeMode="cover" />
      
      <ThemedView style={styles.content}>
        <ThemedText type="title">{product.name}</ThemedText>
        <ThemedText style={styles.category}>{product.category?.name || product.category}</ThemedText>
        <ThemedText style={styles.price}>${typeof product.price === 'number' ? product.price.toFixed(2) : product.price}</ThemedText>
          ...
          {product.reviews && product.reviews.map((review: any) => (
            <ThemedView key={review.id} style={styles.review}>
              <ThemedView style={styles.reviewHeader}>
                ...
                <ThemedText style={styles.reviewRating}>{'★'.repeat(review.rating)}</ThemedText>
              </ThemedView>
        ...
        <Button title="Thêm Vào Giỏ" onPress={handleAddToCart} style={[styles.button, { marginBottom: getTabBarHeight() + 20 }]} />
      </ThemedView>
    </ScrollView>
  );
}
